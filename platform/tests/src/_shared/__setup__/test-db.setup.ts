...
export async function startTestDb(): Promise<DynamoDBClient> {
  ...
  if (!process.env.TABLE_NAME && !process.env.DYNAMODB_CORE_TABLE_NAME) {
    process.env.TABLE_NAME = "oncepass-core-test";
    process.env.DYNAMODB_CORE_TABLE_NAME = "oncepass-core-test";
  }
  if (!process.env.BASE_TABLE_NAME && !process.env.DYNAMODB_BASE_TABLE_NAME) {
    process.env.BASE_TABLE_NAME = "oncepass-base-test";
    process.env.DYNAMODB_BASE_TABLE_NAME = "oncepass-base-test";
  }
  if (!process.env.ANALYTICS_TABLE_NAME && !process.env.DYNAMODB_ANALYTICS_TABLE_NAME) {
    process.env.ANALYTICS_TABLE_NAME = "oncepass-analytics-test";
    process.env.DYNAMODB_ANALYTICS_TABLE_NAME = "oncepass-analytics-test";
  }
